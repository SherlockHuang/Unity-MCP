# compact-tool-results Specification

## Purpose
Define compact execution-result payload contracts for high-volume Unity-MCP tools, currently focused on `tests-run` tree output that reduces repeated test identity text while preserving structured diagnostics.
